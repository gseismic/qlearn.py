...
class BaseAgent(ABC):
    ...
    def learn(self, 
        ...
        while True:
            ...
            if should_exit:
                self.logger.info(get_text(exit_reason, lang))
                break

            if exit_monitor.episode_count % verbose_freq == 0:
                self.logger.info(f"Episode {exit_monitor.episode_count}: {get_text('total_reward', lang)}: {episode_reward}")
            ...
            if checkpoint_freq and exit_monitor.episode_count % checkpoint_freq == 0:
                ...
                self.logger.info(get_text('checkpoint_saved', self.lang) + f': {checkpoint_file}')
        ...
        self.logger.info(get_text('final_model_saved', self.lang) + f': {final_model_path}')
